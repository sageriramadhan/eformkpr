...
export class FormDataAgunan2 extends Component {
  ...
  componentDidMount() {
    LocationService.getDaftarProvinsi(objects => {
      this.setState({ daftarProvinsi: objects })
    })
  }
  ...
  onProvinsiMenuItemClick = provinsi => e => {
    ...
    this.setState({ provinsiTerpilih: provinsi.nama })
    
    LocationService.getDaftarKabupatenKota(provinsi.id, objects => {
      this.setState({ daftarKabupatenKota: objects })
    })
  }

  onKabupatenKotaMenuItemClick = kabupatenKota => e => {
    this.props.handleKabupatenKota(kabupatenKota, 'provinsiAgunan')

    this.setState({ kabupatenKotaTerpilih: kabupatenKota })
    
    LocationService.getDaftarKecamatan(kabupatenKota.id, objects => {
      this.setState({ daftarKecamatan: objects })
    })
  }
  ...
  onKecamatanMenuItemClick = kecamatan => e => {
    ...
    LocationService.getDaftarKelurahan(kecamatan.id, objects => {
      this.setState({ daftarKelurahan: objects })
    })
  }
              ...
              select>
              {
                daftarKabupatenKota.map((kota_kabupaten) =>
                  <MenuItem
                    ...
                    onClick={this.onKabupatenKotaMenuItemClick(kota_kabupaten)}>
                    {kota_kabupaten.nama}
                  </MenuItem>)
              }
            </TextField>
...
export default withStyles(styles)(FormDataAgunan2);
